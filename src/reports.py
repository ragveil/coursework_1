...
from typing import Optional, Callable, Any, LiteralString, Literal
from src.logger import get_logger
from config import ROOT_DIR
import pandas as pd
import json

xlsx_path = os.path.join(ROOT_DIR, "data", "operations.xlsx")
files_path = os.path.join(ROOT_DIR, "logs/")
...
def spending_by_workday(transactions: pd.DataFrame,
    ...
    result = {f'Средние траты за период с {start_date.strftime('%d.%m.%Y')} по {end_date.strftime('%d.%m.%Y')}': {'Траты в рабочие дни': workdays, 'Траты в выходные дни': weekends}}
